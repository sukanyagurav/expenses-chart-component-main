...
function createBar(day,amount,index){
    ...
    inner.className=`bg-softRed w-9 md:w-12 relative rounded-md bar cursor-pointer`
    inner.id=index
    inner.style.height=(amount * 3) +'px'
    div.append(inner)
    const p=document.createElement('p')
    p.className='text-mediumBrown text-sm mt-3'
    p.innerHTML=day
    div.append(p)
...
async function getData(){
    ...
    allBar.forEach((bar)=>{
        ...
        bar.addEventListener('mouseover',function(e){
                ...
                popup.className = 'bg-darkBrown text-cream px-1  py-1 absolute -top-16 -left-2 rounded-md text-[0.9rem] md:text-lg font-bold'
